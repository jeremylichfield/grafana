import {
  ...
  type PluginExtensionElementConfig,
} from '@grafana/data';
...
import {
  ...
  generateExtensionId,
  getEventHelpers,
  isPluginExtensionElementConfig,
} from './utils';
import {
...
export const getPluginExtensions: GetExtensions = ({ context, extensionPointId, registry }) => {
  ...
  for (const registryItem of registryItems) {
    try {
      ...
      // ELEMENT
      if (isPluginExtensionElementConfig(extensionConfig)) {
        // Run the configure() function with the current context, and apply the ovverides
        const overrides = getElementExtensionOverrides(registryItem.pluginId, extensionConfig, frozenContext);
        ...
        const extension: PluginExtensionElement = {
          id: generateExtensionId(registryItem.pluginId, extensionConfig),
          ...
          title: overrides?.title || extensionConfig.title,
          description: overrides?.description || extensionConfig.description,
          element: overrides?.element || extensionConfig.element,
        };
...
function getElementExtensionOverrides(pluginId: string, config: PluginExtensionElementConfig, context?: object) {
  try {
    const overrides = config.configure?.(context);
    ...
    let { title = config.title, description = config.description, element = config.element, ...rest } = overrides;
    ...
    element && assertElementIsValid(element);
    assertStringProps({ title, description }, ['title', 'description']);
